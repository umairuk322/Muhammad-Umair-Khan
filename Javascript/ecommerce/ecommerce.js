...
function renderProducts(data){
  let mappedProducts = data.map((meriProduct)=>{
    return  `<div class="product">
          <img
            src=${meriProduct.images[0]}
            alt="product"
          />
          <h4>${meriProduct.category}</h4>
          <div class="product-header">
            <h4>${meriProduct.title}</h4>
            <div class="product-price">$${meriProduct.price}</div>
          </div>
...
function getFilteredProducts(productCategory) {
  let filteredProducts = data.products.filter((meriProduct)=>{
    return meriProduct.category===productCategory
  })
  let mappedProducts = filteredProducts.map((meriProduct)=>{
    return `<div class="product">
          <img
            src=${meriProduct.images[0]}
            alt="product"
  ...
  products.innerHTML = mappedProducts
}
renderProducts(filteredProducts)
function searchProducts(){
  let searchTerm = document.querySelector('input[name="search"]').value.toLowerCase()
  console.log("user is typing", searchTerm)
  const filteredProducts = data.products.filter((meriProduct)=>{
    return meriProduct.title.toLowerCase().startsWith(searchTerm) || meriProduct.description.toLowerCase().startsWith(searchTerm)
  })
}
renderProducts(filteredProducts)
